...
async function handleRequest(request) {
  ...
  if (request.method === "POST") {
    ...
    return new Response(`{"status":500,"key":": 오류: Url 정보가 사기성이거나 바이러스가 있거나 올바르지 않습니다."}`, {
  ...
  if(!path){
    ...
    const html= await fetch("https://tira-tw.github.io/korea-url/"+config.theme+"/index.html")
