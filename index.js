const config = {
  ...
  <body>
    ...
    <a href="https://github.com/datasense-gh/Url-Shorten-Worker/" target="_self">Fork me on GitHub</a>
  </body>`
  ...
  　　len = len || 6;
  　　let $chars = 'ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678';    /**** The easily confused characters oOLl, 9gq, Vv, Uu, I1 are removed by default. ****/
  ...
  async function handleRequest(request) {
    ...
    if(!path){
      ...
      const html= await fetch("https://datasense-gh.github.io/Url-Shorten-Worker/"+config.theme+"/index.html")
    ...
    if (location) {
      if (config.safe_browsing_api_key){
        if(!(await is_url_safe(location))){
          let warning_page = await fetch("https://datasense-gh.github.io/Url-Shorten-Worker/safe-browsing.html")
      ...
      if (config.no_ref=="on"){
        let no_ref= await fetch("https://datasense-gh.github.io/Url-Shorten-Worker/no-ref.html")
